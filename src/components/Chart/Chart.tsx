...
export default function Chart() {
  ...
  const [localGrid, setLocalGrid] = useState<number[][] | undefined>();
  const [globalGrid, setGlobalGrid] = useState<number[][] | undefined>();
  const [localArray, setLocalArray] = useState<number[] | undefined>([]);
  const [globalArray, setGlobalArray] = useState<number[] | undefined>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const l: SessionData[] | undefined = await d3.json("/datasets/local-heatmap.json");
        setLocalData(l);
        const g: SessionData[] | undefined = await d3.json("/datasets/global-heatmap.json");
        setGlobalData(g);
  ...
  function getGrid(data: SessionData[]): number[][] {
    const grid: number[][] = Array.from({length: 100}, () => Array.from({length: 100}, () => 0));
  ...
  useEffect(() => {
    ...
    const plot = Plot.plot({
      ...
      marks: [
        ...
        Plot.density(localData, {
          x: "x", y: "y", opacity: 1, bandwidth, fill: "density", stroke: "white", strokeOpacity: 0.3
        }),
        ...
        Plot.text(emotions, {x: (d: Emotion) => d.position[0], y: (d: Emotion) => d.position[1], text: (d:Emotion) => d.name, lineAnchor: "bottom", dy: -8 }),
        Plot.dot(emotions, {x: (d: Emotion) => d.position[0], y: (d: Emotion) => d.position[1], fill: (d:Emotion) => toRgb(d.color!) }),
      ]
    });
    ...
    return () => plot.remove();
  }, [localData, globalData, localArray, globalArray, localGrid, globalGrid, opacity, bandwidth, displayPoints]);
  ...
  return (
    <>
      <div className={'gui'}>
        ...
        <div>
          <label>
            Opacity: <input type="range" min="0" max="1" step="0.1" value={opacity} onChange={handleOpacityChange}/>
          </label>
        </div>
        <div>
          <label>
            Bandwidth: <input type="range" min="10" max="80" step="2" value={bandwidth}
